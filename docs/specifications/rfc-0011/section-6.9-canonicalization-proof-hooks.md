...
### Idempotence requirement (INV-1)

**CANON MUST enforce idempotence:**

For any CLBC-POLY frame `P`, after canonicalization:
- `CANON(CANON(P))` MUST produce byte-identical output to `CANON(P)`
- This MUST hold regardless of the initial representation of `P`

**Operational requirement:**
- VM implementations MUST verify idempotence during testing
- VM MAY optimize by checking if input is already canonical before processing
- If input is already canonical, output MUST be byte-identical to input

**Invariant reference:** This enforces INV-1 (Normalization Idempotence) from `TETRAGRAMMATRON_OS_FORMAL_INVARIANTS.md`.

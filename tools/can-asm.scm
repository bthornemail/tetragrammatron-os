;; tools/can-asm.scm
;; CANB v1 assembler (RFC-0012 §4, enforces 16-byte fixed-width instructions)
  ...
  (cond
    ((and (pair? form) (eq? (car form) 'flags))
     (let* ((flags (cdr form))
       ...
       ;; reserved bits must be 0 (RFC-0012 §4.3)
...
(define (assemble-forms forms)
  (append*
    (map (lambda (f)
           (cond
             ((and (pair? f) (eq? (car f) 'inst) (= (length f) 8))
              (let ((op (cadr f))
                    (flags (caddr f))
                    (rdst (cadddr f))
                    (ra (list-ref f 4))
             ...
             (else
              (die "expected (inst OP FLAGS RDST RA RB IMM16 REF32), got ~s" f))))
         forms)))
    ...
    (lambda (in)
      (let* ((forms (read-all-sexps in))
        ...
        (call-with-output-file out-path
          ...
          #:exists 'replace)
        (printf "Wrote ~a bytes to ~a~%" (length bytes) out-path)))))
